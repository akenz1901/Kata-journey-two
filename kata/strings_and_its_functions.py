...
class PythonStrings(object):

    def display_string(self, name, age):
    ...
    def display_string_using_format_order():
        ...
        print(
            "They both bearing {2} and surname is {0} maybe the other one middle name is {1}".format(names[0], names[2],
                                                                                                     names[1]))
    ...
    def contains(self, word):
        ...
        count = 1
        while len(word) < len(self.word) or end <= len(self.word):
            if word == self.word[start:end]:
                return True
            start += 1
            end += 1
            count += 1
